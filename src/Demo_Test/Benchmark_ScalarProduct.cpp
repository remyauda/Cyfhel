/*
#   Benchmark_ScalarProduct
#   --------------------------------------------------------------------
#   Perform tests on operation Scalar Product. 
#   --------------------------------------------------------------------
...
int main(int argc, char *argv[])
{
    ...
    std::cout <<"" <<endl;
    std::cout <<"     ************Benchmark_ScalarProduct************" <<endl;
    std::cout <<"" <<endl;
	...
	{
		std::cout <<"******Perform Scalar Product operation "<< k+1 <<"******"<<endl<<endl;

		// Begin the chrono.
		Timer timerDemo(true);
		timerDemo.start();

		//Perform Scalar Product operation
		// Scalar product with returnScalarProd method of the two cypher text.
		CyCtxt cScalarProd1_2 = c1.returnScalarProd(c2);
	...
	LibMatrix::writeDoubleInFileWithEraseData("Result_Benchmark_ScalarProduct", averageOfExecutionTime);// Write the double averageOfExecutionTime in the file Result_Benchmark_CreateCyfhel in the directory ResultOfBenchmark.

	LibMatrix::writeStringInFileWithEraseData("ResultVerbose_Benchmark_ScalarProduct", LibMatrix::transformSecondToYearMonthWeekHourMinSecMilli(averageOfExecutionTime));// Write the string verbose to transform the average of execution time in seconds to string verbose Years, Months, Weeks, Hours, Minutes, Seconds, Milliseconds in the file ResultVerbose_Benchmark_CreateCyfhel in the directory ResultOfBenchmark.

    // Skip a line.
    std::cout <<"\n"<<endl;

    // Display the end of the program.
    std::cout <<"     ************End of Benchmark_ScalarProduct************" <<endl;
