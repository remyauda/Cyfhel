...
int main(int argc, char **argv)
{
    ...
    */
	...
	long m = -1;                  // Specific modulus
	long p = 2;                   // Plaintext base [default=2], should be a prime number
	long r = 32;                  // Lifting [default=1]
	long L = 40;                  // Number of levels in the modulus chain [default=heuristic]
	long c = 2;                   // Number of columns in key-switching matrix [default=2]
	long w = 64;                  // Hamming weight of secret key
	long d = 1;                   // Degree of the field extension [default=1]
	...
	*/
	...
	std::cout << " Result2: " << method2Result << ". Result3: " << method3Result << std::endl;	
    ...
    //Cyfhel cy(true);
    Cyfhel cy(true, p, r, c, d, k, w, L, m);
	...
	tInitCustom.stop();
	std::cout << "Time taken for the initialization: " << tInitCustom.elapsed_time() << std::endl;
	


	/*** METHOD 1 CUSTOM ***/
	Timer tMethodCustom1;
	tMethodCustom1.start();
    CyCtxt c12 = cy.encrypt(v1);
    CyCtxt c22 = cy.encrypt(v2);
    CyCtxt cScalarProd12_22 = c12.returnScalarProd(c22);
    vector<long> v_scalarProd_v12_v22 = cy.decrypt(cScalarProd12_22);
    tMethodCustom1.stop();
	std::cout << "Custom method result: " << v_scalarProd_v12_v22[0] << ". Done in " << tMethodCustom1.elapsed_time() << "s." <<  std::endl;	

    std::cout <<"Decrypt(Encrypt(v12).returnScalarProd(Encrypt(v22)) -> "<< v_scalarProd_v12_v22[0] <<endl;

	/*** METHOD 2 CUSTOM ***/
	/*Timer tMethodCustom2;
	tMethodCustom2.start();
    vector<CyCtxt> vectorc1c2 = cy.encrypt(v1, v2);
    CyCtxt cScalarProdInvert = vectorc1c2[0].invertAndMultiplyScalarProd(vectorc1c2[1]);
    vector<long> v_scalarProdByInvert = cy.decrypt(cScalarProdInvert);
    tMethodCustom2.stop();
	std::cout << "Custom method result: " << v_scalarProdByInvert << ". Done in " << tMethodCustom2.elapsed_time() << "s." <<  std::endl;
	*/
